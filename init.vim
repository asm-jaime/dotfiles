...
let s:semantic_search_job = v:null
let s:semantic_search_output = []
let s:semantic_search_errors = []
let s:semantic_search_root = ''
let s:semantic_search_query = ''

function! s:GrepaiExecutable() abort
  let grepai_path = exepath('grepai')
  if !empty(grepai_path)
    return grepai_path
  endif

  let user_executable = expand('~/.local/bin/grepai')
  return executable(user_executable) ? user_executable : ''
endfunction

function! s:CollectSemanticSearchOutput(channel, message) abort
  call add(s:semantic_search_output, a:message)
endfunction

function! s:CollectSemanticSearchError(channel, message) abort
  if !empty(a:message)
    call add(s:semantic_search_errors, a:message)
  endif
endfunction

function! s:SemanticResultPath(result) abort
  for key in ['file_path', 'path', 'file']
    let path = get(a:result, key, '')
    if !empty(path)
      return path
    endif
  endfor
...
function! s:FinishSemanticSearch(job, status) abort
  if a:status != 0
    echohl ErrorMsg
    echomsg 'grepai search failed: '
          \ . (empty(s:semantic_search_errors)
          \   ? 'exit status ' . a:status
          \   : join(s:semantic_search_errors, ' '))
    echohl None
    return
  endif

  try
    let payload = json_decode(join(s:semantic_search_output, "\n"))
  catch
    echohl ErrorMsg
    echomsg 'grepai returned invalid JSON: ' . v:exception
    echohl None
    return
  endtry

  let results = type(payload) == v:t_dict
        \ ? get(payload, 'results', [])
        \ : payload
  ...
  for result in results
    ...
    let path = s:SemanticResultPath(result)
    if empty(path)
      continue
    endif

    let content = get(result, 'content', get(result, 'text', ''))
    let content_lines = filter(
          \ split(content, "\n"),
          \ {_, line -> !empty(trim(line)) && line !~# '^File: '})
    let first_line = empty(content_lines)
          \ ? printf('semantic match ending at line %d',
          \     get(result, 'end_line', get(result, 'line_end', 0)))
          \ : trim(content_lines[0])
    let score = get(result, 'score', get(result, 'similarity', 0.0))
    call add(items, {
          \ 'filename': fnamemodify(s:semantic_search_root . '/' . path, ':p'),
          \ 'lnum': get(result, 'start_line',
          \     get(result, 'line_start', get(result, 'line', 1))),
          \ 'text': printf('[%.3f] %s', score, first_line),
          \})
  endfor

  call setqflist([], 'r', {
        \ 'title': 'grepai: ' . s:semantic_search_query,
        \ 'items': items,
        \})
  if empty(items)
    echomsg 'grepai: no semantic matches'
    return
  endif

  " Vimspector's optional BufNew hook raises E495 for Vim's nameless first
  " quickfix buffer. It does not prevent the window from opening.
  silent! botright copen
  echomsg printf('grepai: %d semantic matches', len(items))
endfunction

function! s:SemanticSearch(query) abort
  let query = empty(a:query) ? input('Semantic search: ') : a:query
  if empty(query)
    return
  endif

  let grepai = s:GrepaiExecutable()
  if empty(grepai)
    echohl ErrorMsg
    echomsg 'grepai is not installed; run install-grepai.sh'
    echohl None
    return
  endif

  let root = s:ProjectRoot()
  if !filereadable(root . '/.grepai/config.yaml')
    echohl ErrorMsg
    echomsg 'grepai is not initialized here; run grepai-project-init at the project root'
    echohl None
    return
  endif

  if type(s:semantic_search_job) == v:t_job
        \ && job_status(s:semantic_search_job) ==# 'run'
    call job_stop(s:semantic_search_job)
  endif

  let s:semantic_search_output = []
  let s:semantic_search_errors = []
  let s:semantic_search_root = root
  let s:semantic_search_query = query
  let s:semantic_search_job = job_start(
        \ [grepai, 'search', query, '--json', '--limit', '20'],
        \ {
        \   'cwd': root,
        \   'out_mode': 'nl',
        \   'out_cb': function('<SID>CollectSemanticSearchOutput'),
        \   'err_mode': 'nl',
        \   'err_cb': function('<SID>CollectSemanticSearchError'),
        \   'exit_cb': function('<SID>FinishSemanticSearch'),
        \ })

  if job_status(s:semantic_search_job) ==# 'fail'
    echohl ErrorMsg
    echomsg 'grepai search could not be started'
    echohl None
    return
  endif
  echomsg 'grepai: searching project index...'
endfunction

command! ProjectFiles call <SID>FindProjectFile()
command! -nargs=? ProjectSearch call <SID>SearchProject(<q-args>)
command! -nargs=* SemanticSearch call <SID>SemanticSearch(<q-args>)
...
nnoremap <silent> <leader>s :SemanticSearch<CR>
